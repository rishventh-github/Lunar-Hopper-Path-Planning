...
def check_ballistic_hop(start_node, end_node, terrain, grid_res=1000.0, g=1.62, search_mode=True):
    r0, c0 = start_node
    r1, c1 = end_node
    x0, y0 = c0 * grid_res, r0 * grid_res
    x1, y1 = c1 * grid_res, r1 * grid_res
    ...
    total_horizontal_dist = np.sqrt((x1 - x0)**2 + (y1 - y0)**2)
    if total_horizontal_dist == 0:
        return True, {"x": [x0], "y": [y0], "z": [z0]}, 0.0, 0.0
        
    target_apex_z = max(z0, z1) + 100.0
    if target_apex_z > (MAX_ALLOWABLE_ALTITUDE - 50.0):
        target_apex_z = MAX_ALLOWABLE_ALTITUDE - 50.0 # Enforce strict 1km ceiling buffer

    if z0 >= MAX_ALLOWABLE_ALTITUDE or z1 >= MAX_ALLOWABLE_ALTITUDE:
        return False, None, 0.0, 0.0
    ...
    if flight_time > MAX_ALLOWABLE_TIME: return False, None, 0.0, 0.0

    v_x = total_horizontal_dist / flight_time
    v_z0 = g * t_rise
    v0 = np.sqrt(v_x**2 + v_z0**2)
    delta_v = v0 + np.sqrt(v_x**2 + (g * t_fall)**2)
    c_exhaust = 400 * 9.81
    fuel_mass_burned = HOPPER_MASS * (np.exp(delta_v / c_exhaust) - 1)
    ...
    for s in samples:
        ...
        if current_z > MAX_ALLOWABLE_ALTITUDE: return False, None, 0.0, 0.0
        r_map = int(np.clip(np.round((y0 + ratio*(y1-y0)) / grid_res), 0, terrain.shape[0] - 1))
        c_map = int(np.clip(np.round((x0 + ratio*(x1-x0)) / grid_res), 0, terrain.shape[1] - 1))
        
        if 10.0 < s < (total_horizontal_dist - 10.0):
            if current_z <= terrain[r_map, c_map]: return False, None, 0.0, 0.0  
                
        traj_x.append(x0 + ratio*(x1-x0))
        traj_y.append(y0 + ratio*(y1-y0))
        traj_z.append(current_z)
...
def get_neighbors(node, map_size):
    ...
    for dr in range(-max_cell_radius, max_cell_radius + 1, 3):
        for dc in range(-max_cell_radius, max_cell_radius + 1, 3):
            ...
            r_new, c_new = r_curr + dr, c_curr + dc
            if 0 <= r_new < map_size and 0 <= c_new < map_size:
                if slope_map[r_new, c_new] > MAX_ALLOWABLE_SLOPE or vrm_map[r_new, c_new] > MAX_ALLOWABLE_VRM: continue 
                distance = np.sqrt(dr**2 + dc**2) * GRID_RESOLUTION
                if distance <= MAX_HOP_DISTANCE:
                    neighbors.append(((r_new, c_new), distance))
            
    # FIXED: Re-locked target checking to our single vehicle endpoint coordinates
    end_node = (125, 135) 
    dist_to_goal = np.sqrt((end_node[0] - r_curr)**2 + (end_node[1] - c_curr)**2) * GRID_RESOLUTION
    if dist_to_goal <= MAX_HOP_DISTANCE:
        if slope_map[end_node[0], end_node[1]] <= MAX_ALLOWABLE_SLOPE and vrm_map[end_node[0], end_node[1]] <= MAX_ALLOWABLE_VRM:
            neighbors.append((end_node, dist_to_goal))
...
#             is_clear, _, fuel_cost, flight_time = check_ballistic_hop(current, neighbor, terrain_map, grid_res=GRID_RESOLUTION, g=GRAVITY_MOON, search_mode=True, agent_ceiling=agent_ceiling_limit)
#             if not is_clear:
...
#             hop_cost = movement_cost + risk_penalty + time_penalty + communication_penalty + illumination_penalty + terrain_penalty + compute_penalty
...
def waypoint_path_planning(start, goal):
    open_set = []
    # Queue tracks standard 2D coordinates: (f_score, node_coordinate, running_fuel)
    heapq.heappush(open_set, (0, start, FUEL_CAPACITY))
    came_from = {}
    closed_set = set()
    search_history = [] 

    g_score = {p: float('inf') for p in np.ndindex((MAP_SIZE, MAP_SIZE))}
    g_score[start] = 0
    
    f_score = {p: float('inf') for p in np.ndindex((MAP_SIZE, MAP_SIZE))}
    f_score[start] = np.sqrt((start[0] - goal[0])**2 + (start[1] - goal[1])**2) * GRID_RESOLUTION

    fuel_spent_map = {p: 0.0 for p in np.ndindex((MAP_SIZE, MAP_SIZE))}
    fuel_spent_map[start] = FUEL_CAPACITY
    ...
    while open_set:
        _, current, current_fuel_remaining = heapq.heappop(open_set)
        nodes_evaluated += 1
        ...
        if current == goal:
            ...
            while current in came_from:
                ...
                path.append(current)
            return path[::-1], fuel_spent_map, search_history

        if current in closed_set: continue
        closed_set.add(current)
        
        parent_node = came_from.get(current, None)
        search_history.append((current, parent_node))

        # Standard single-agent 2-parameter neighbors retrieval
        for neighbor, distance in get_neighbors(current, MAP_SIZE):
            r_new, c_new = neighbor

            # Kinematic velocity pruning gate
            approx_v_horizontal = distance / MAX_ALLOWABLE_TIME
            if approx_v_horizontal > 600.0: continue

            # Lazy evaluation line-of-sight filter
            mid_r = int((current[0] + r_new) // 2)
            mid_c = int((current[1] + c_new) // 2)
            if terrain_map[mid_r, mid_c] > terrain_map[current[0], current[1]] + 900.0:
                fail_los_filter += 1
            ...
            is_clear, _, fuel_cost, flight_time = check_ballistic_hop(current, neighbor, terrain_map, GRID_RESOLUTION, GRAVITY_MOON, search_mode=True)
            if not is_clear:
                fail_ballistic_calc += 1
                continue 
            ...
            distance_cost = distance * waypoint_distance_weight
            wasted_range = MAX_HOP_DISTANCE - distance
            hop_efficiency_penalty = (wasted_range * 10.0) * waypoint_distance_weight
            movement_cost = distance_cost + hop_efficiency_penalty 
            ...
            communication_penalty = (1.0 - communication_map[r_new, c_new]) * MAX_HOP_DISTANCE * waypoint_communication_weight
            illumination_penalty = (1.0 - illumination_map[r_new, c_new]) * MAX_HOP_DISTANCE * waypoint_illumination_weight
            terrain_penalty = abs(terrain_map[r_new, c_new] - terrain_map[current[0], current[1]]) * 10.0 * waypoint_terrain_weight
            compute_penalty = len(closed_set) * 0.05 
            ...
            if tentative_g_score < g_score[neighbor]:
                ...
                fuel_spent_map[neighbor] = candidate_remaining_fuel  
                ...
                heapq.heappush(open_set, (f_score[neighbor], neighbor, candidate_remaining_fuel))
...
print(f"\nVerifying Launch Pad Safety...")
print(f" -> Start Node {start_node} Slope: {slope_map[start_node[0], start_node[1]]:.1f}° | VRM: {vrm_map[start_node[0], start_node[1]]:.4f}")
print(f" -> End Node   {end_node} Slope: {slope_map[end_node[0], end_node[1]]:.1f}° | VRM: {vrm_map[end_node[0], end_node[1]]:.4f}")

print("\nExecuting single-agent path planning session...")
tic = time.time()
waypoints, fuel_spent_map, search_history = waypoint_path_planning(start_node, end_node)
toc = time.time()
total_planning_time = toc - tic
print(f"Path planning computation successfully finished in {total_planning_time:.4f} seconds.")
...
# ========================================================================
# 5. Single-Agent Flight Analytics Summary Report & Visualizations
# ========================================================================
if waypoints and fuel_spent_map is not None:
    waypoints_x = [p[1] for p in waypoints]  # Columns map cleanly to X-Axis
    waypoints_y = [p[0] for p in waypoints]  # Rows map cleanly to Y-Axis
    ...
    print("\n================== MISSION TRAJECTORY SUMMARY REPORT ==================")
    print(f"Algorithm Execution Planning Latency: {total_planning_time:.4f} seconds.")
    print("------------------------------------------------------------------------")
    ...
    for idx, pt in enumerate(waypoints):
        ...
        if idx == 0:
            print(f" -> Launch Node: Coordinates ({row}, {col})")
        else:
            prev_node = waypoints[idx-1]
            # Pristine high-precision recalculation step for your final diagnostic readouts
            is_clear, arc, hop_fuel, hop_time = check_ballistic_hop(prev_node, pt, terrain_map, GRID_RESOLUTION, GRAVITY_MOON, search_mode=False)
            
            # Kinematic data recovery fallback block for rugged high-slope peaks
            if hop_fuel == 0.0 or hop_time == 0.0:
                hop_dist_m = np.sqrt((row - prev_node[0])**2 + (col - prev_node[1])**2) * GRID_RESOLUTION
                t_rise_f = np.sqrt(2 * max(0.1, (MAX_ALLOWABLE_ALTITUDE - 100.0 - terrain_map[prev_node[0], prev_node[1]])) / GRAVITY_MOON)
                t_fall_f = np.sqrt(2 * max(0.1, (MAX_ALLOWABLE_ALTITUDE - 100.0 - terrain_map[row, col])) / GRAVITY_MOON)
                hop_time = t_rise_f + t_fall_f
                v_x_f = hop_dist_m / hop_time
                v_z0_f = GRAVITY_MOON * t_rise_f
                v0_f = np.sqrt(v_x_f**2 + v_z0_f**2)
            ...
            total_fuel_mass_burned += hop_fuel
            total_airborne_time += hop_time
            print(f" -> Hop {idx}: To ({row}, {col}) | Dist: {hop_dist/1000.0:.1f} km | Airtime: {hop_time:.1f} s | Fuel Used: {hop_fuel:.2f} kg | Local Risk: {node_risk:.1f}")
            
    final_remaining_fuel = fuel_spent_map[end_node]
    print(f" -> Total Hops Taken       : {len(waypoints) - 1} Jumps")
    print(f" -> Total Journey Distance : {total_route_distance/1000.0:.1f} km")
    print(f" -> Propellant Remaining   : {final_remaining_fuel:.2f} kg / {FUEL_CAPACITY:.1f} kg")
    print(f" -> Total Airborne Duration: {total_airborne_time:.1f} seconds")
    print(f" -> Mission Asset Risk Sum : {accumulated_risk_score:.2f} Cumulative Risk Points")

    # ========================================================================
    # FIRST: DRAW AND RENDERING THE 2D PLOTS
    # ========================================================================
    fig, axs = plt.subplots(2, 3, figsize=(18, 10))
    ...
    axs[0, 0].plot(waypoints_x, waypoints_y, '-ro', markersize=6, label='Waypoints Planned')
    axs[0, 0].scatter([start_node[1], end_node[1]], [start_node[0], end_node[0]], color='blue', s=100, zorder=5, label='Start/End')
    axs[0, 0].set_title("Waypoint Route Planning on Terrain Map", weight='bold')
    axs[0, 0].legend()
    fig.colorbar(im0, ax=axs[0, 0], label='Elevation (m)')

    # Communication link map profile
    im1 = axs[0, 1].imshow(communication_map, cmap='plasma', origin='lower')
    axs[0, 1].plot(waypoints_x, waypoints_y, '-wo', markersize=6)
    axs[0, 1].scatter([start_node[1], end_node[1]], [start_node[0], end_node[0]], color='blue', s=100, zorder=5)
    axs[0, 1].set_title("Communication Signal Strength Map", weight='bold')
    fig.colorbar(im1, ax=axs[0, 1], label='Signal strength (0-1)')

    # Illumination grid array chart
    im2 = axs[0, 2].imshow(illumination_map, cmap='YlOrRd_r', origin='lower')
    axs[0, 2].plot(waypoints_x, waypoints_y, '-ko', markersize=6)
    axs[0, 2].scatter([start_node[1], end_node[1]], [start_node[0], end_node[0]], color='blue', s=100, zorder=5)
    axs[0, 2].set_title("Illumination Map", weight='bold')
    fig.colorbar(im2, ax=axs[0, 2], label='Illumination Level (0-1)')

    # Gradients slope chart
    im3 = axs[1, 0].imshow(slope_map, cmap='magma', origin='lower')
    axs[1, 0].plot(waypoints_x, waypoints_y, '-wo', markersize=6)
    axs[1, 0].scatter([start_node[1], end_node[1]], [start_node[0], end_node[0]], color='blue', s=100, zorder=5)
    axs[1, 0].set_title("Surface Landing Slope Map", weight='bold')
    fig.colorbar(im3, ax=axs[1, 0], label='Angle (Degrees °)')

    # Surface roughness profile sheet
    im4 = axs[1, 1].imshow(vrm_map, cmap='copper', origin='lower')
    axs[1, 1].plot(waypoints_x, waypoints_y, '-co', markersize=6)
    axs[1, 1].scatter([start_node[1], end_node[1]], [start_node[0], end_node[0]], color='blue', s=100, zorder=5)
    axs[1, 1].set_title("Vector Ruggedness Map (VRM)", weight='bold')
    fig.colorbar(im4, ax=axs[1, 1], label='VRM Value (0-1)')

    # Clear text scoreboard status textbox pane
    axs[1, 2].axis('off')
    axs[1, 2].text(0.1, 0.5, f"MISSION STATUS: SUCCESS\n\n"
                  f"Total Waypoints: {len(waypoints)}\n"
                  f"Total Jumps: {len(waypoints)-1}\n\n"
                  f"Constraints Enforced:\n"
                  f" - Max Slope: {MAX_ALLOWABLE_SLOPE}°\n"
                  f" - Max VRM: {MAX_ALLOWABLE_VRM}\n"
                  f" - Sky Ceiling: {MAX_ALLOWABLE_ALTITUDE/1000.0:.1f} km\n\n"
                  f"Fuel Expended: {total_fuel_mass_burned:.2f} kg\n"
                  f"Airtime Duration: {total_airborne_time:.1f} s\n"
                  f"Planning Latency: {total_planning_time:.4f} s", 
                  fontsize=11, weight='bold', verticalalignment='center',
                  bbox=dict(facecolor='white', alpha=0.8, boxstyle='round,pad=1'))

    plt.tight_layout()
    # SHOW THE 2D PLOTS GRID SHEET INSTANTLY BEFORE RUNNING THE 3D POPUP WINDOW
    plt.show(block=False) 

    # ========================================================================
    # SECOND: GENERATING AND SHOWING THE 3D BALLISTIC CORRIDORS WINDOW
    # ========================================================================
    def visualize_3d_trajectory(waypoints, terrain_map, grid_res=1000.0, g=1.62):
        fig3d = plt.figure(figsize=(13, 9))
        ax3d = fig3d.add_subplot(111, projection='3d')
        
        # Build surface topology mesh
        rows, cols = terrain_map.shape
        X_m, Y_m = np.meshgrid(np.arange(cols) * grid_res, np.arange(rows) * grid_res)
        surf = ax3d.plot_surface(X_m, Y_m, terrain_map, cmap='terrain', alpha=0.5, edgecolor='none', zorder=1)
        
        # Transparent ceiling plane shield
        ceiling_z = np.full_like(X_m, MAX_ALLOWABLE_ALTITUDE)
        ax3d.plot_surface(X_m, Y_m, ceiling_z, color='red', alpha=0.1, edgecolor='none', zorder=2)
        ...
        for i in range(len(waypoints) - 1):
            ...
            # Use high precision mode for drawings
            is_clear, arc, _, _ = check_ballistic_hop(start, end, terrain_map, grid_res, g, search_mode=False)
            
            # Math parabola fallback engine to guarantee lines connect on your canvas
            if arc is None:
                x0, y0 = start[1] * grid_res, start[0] * grid_res
                x1, y1 = end[1] * grid_res, end[0] * grid_res
                z0, z1 = terrain_map[start[0], start[1]], terrain_map[end[0], end[1]]
                ...
                if dist > 0:
                    ...
                    for s in t_samples:
                        ratio = s / dist
                        t_el = s / v_x_fallback
                        arc["x"].append(x0 + ratio * dx)
                        arc["y"].append(y0 + ratio * dy)
                        arc["z"].append(z0 + v_z0_fallback * t_el - 0.5 * g * (t_el**2))

            if arc and "x" in arc:
                ax3d.plot(arc["x"], arc["y"], arc["z"], color='red', linewidth=3, zorder=10,
                          label="Flight Arc" if not arc_labeled else "")
                arc_labeled = True
                
        # FIXED: Explicitly index simple list tuples: index 0 is row, index 1 is column
        wp_rows = [p[0] for p in waypoints]
        wp_cols = [p[1] for p in waypoints]
        
        wp_x = [c * grid_res for c in wp_cols]  # Columns to X
        wp_y = [r * grid_res for r in wp_rows]  # Rows to Y
        wp_z = [terrain_map[r, c] for r, c in zip(wp_rows, wp_cols)]
        
                # 3. Draw landmarks and landing pads onto the 3D grid surface mesh
        ax3d.scatter(wp_x, wp_y, wp_z, color='darkorange', s=60, edgecolors='black', depthshade=False, zorder=12, label="Waypoints")
        ax3d.scatter([wp_x[0]], [wp_y[0]], [wp_z[0]], color='blue', s=120, edgecolors='black', depthshade=False, zorder=15, label="Start Pad")
        ax3d.scatter([wp_x[-1]], [wp_y[-1]], [wp_z[-1]], color='magenta', s=120, edgecolors='black', depthshade=False, zorder=15, label="Target Goal")
        
        # 4. Viewport boundaries and stretched aspect scaling configurations
        max_boundary = MAP_SIZE * grid_res
        ax3d.set_xlim(0, max_boundary)
        ax3d.set_ylim(0, max_boundary)
        ax3d.set_zlim(0, MAX_ALLOWABLE_ALTITUDE + 1000.0)
        ax3d.set_box_aspect((1, 1, 0.45))
        ax3d.view_init(elev=28, azim=-55)
        
        # 5. Graph titles, descriptive axis labels, colorbar, and legends
        ax3d.set_title("3D Sub-Orbital Ballistic Path Profile Across Terrain", fontsize=13, weight='bold')
        ax3d.set_xlabel("X Space (m)")
        ax3d.set_ylabel("Y Space (m)")
        ax3d.set_zlabel("Z Altitude (m)")
        fig3d.colorbar(surf, ax=ax3d, shrink=0.45, label='Elevation (m)')
        ax3d.legend(loc="upper left")
        plt.show()

    # Launch the single-agent trajectory window
    visualize_3d_trajectory(waypoints, terrain_map, GRID_RESOLUTION, GRAVITY_MOON)
else:
    print("\n[!] Path Planning failed: No trajectory found satisfying concurrent constraints.")
