...
INDICES_PER_FRAME = 2
TARGET_WARNING_SECONDS = 5.0
LOOKAHEAD_INDICES = int(TARGET_WARNING_SECONDS * FRAMES_PER_SECOND * INDICES_PER_FRAME)
...
ROVER_FUEL_CAPACITY = {
    1: 1500.0,
    2: 1500.0,
    3: 2200.0, # more fuel to navigate around pillars
    4: 1500.0
}
...
terrain_map[65:80, 46:54] = terrain_map[65:80, 46:54]
...
for i, r in enumerate([63, 64]):
    ramp_height = 800.0 * (i + 1) / 3.0
    terrain_map[r, 46:54] = 150.0 + ramp_height
    vrm_map[r, 46:54] = 0.005


for i, r in enumerate([80, 81]): # ramp down going out
    ramp_height = 800.0
    terrain_map[r, 46:54] = 150.0 + ramp_height
    vrm_map[r, 46:54] = 0.005


# fix VRM spike sitting on row 70
vrm_map[70, 46:54] = 0.008
vrm_map[75, 46:54] = 0.008


# soften pillar edges into ramps
for col_range in [(46, 54), (96, 104)]:
    c_lo, c_hi = col_range
    for i, r in enumerate([63, 64]):
        terrain_map[r, c_lo:c_hi] = 150.0 + 800.0 * (i + 1) / 3.0
        vrm_map[r, c_lo:c_hi] = 0.005
    for i, r in enumerate([80, 81]):
        terrain_map[r, c_lo:c_hi] = 150.0 + 800.0 * (2 - i) / 3.0
        vrm_map[r, c_lo:c_hi] = 0.005
    vrm_map[70, c_lo:c_hi] = 0.008
    vrm_map[75, c_lo:c_hi] = 0.008


# print("\nDiagonistic: Checking rows 55-90 at column 50 (inside pillar range):")
# for r in range(55, 90):
#     c = 50
#     print(f"row {r}: slope={slope_map[r, c]:.1f} (max {MAX_ALLOWABLE_SLOPE}), "
#           f"vrm={vrm_map[r, c]:.4f} (max {MAX_ALLOWABLE_VRM})")
...
def check_ballistic_hop(start_node, end_node, terrain, grid_res=1000.0, g=1.62, search_mode=True, agent_ceiling=1500.0):
    ...
    n_cells_crossed = max(abs(r1 - r0), abs(c1 - c0), 1)
    prescan_count = max(10, n_cells_crossed + 1)
    ...
    for s in prescan_samples:
        ...
        prescan_terrain.append(float(terrain[r_map, c_map]))
        # ratio = s / total_horizontal_dist
        # current_x = x0 + ratio * dx_space
        # current_y = y0 + ratio * dy_space
        # r_map = int(np.clip(np.round(current_y / grid_res), 0, terrain.shape[0] - 1))
    ...
    max_terrain_along_path = max(max(prescan_terrain), z0, z1)
    target_apex_z = max_terrain_along_path + 200.0 # standard safety buffer clearance <1000 m
    if target_apex_z > (agent_ceiling - 50.0):
        target_apex_z = agent_ceiling - 50.0


    # raise apex until parabola clears every prescanned point
    for _ in range(15):
        ...
        if flight_time <= 0:
            break
        v_x_check = total_horizontal_dist / flight_time
        v_z0_check = g * t_rise


        worst_violation = 0.0
        for s, terr in zip(prescan_samples, prescan_terrain):
            t_elapsed = s / v_x_check
            traj_z = z0 + v_z0_check * t_elapsed - 0.5 * g * (t_elapsed**2)
            clearance = traj_z - terr
            if clearance < 50.0:
                worst_violation = max(worst_violation, 50.0 - clearance)
        if worst_violation <= 0:
            break # trajectory clears everywhere


        target_apex_z += worst_violation + 20.0
        if target_apex_z > (agent_ceiling - 50.0):
            target_apex_z = agent_ceiling - 50.0
            break
    ...
    for s in samples:
        ...
        if 10.0 < s < (total_horizontal_dist - 10.0):
            if current_z <= terrain_z:
                # print(f"[Crash Debug] s={s:.1f}/{total_horizontal_dist:.1f} | "
                #       f"current_z={current_z:.1f} terrain_z={terrain_z:.1f} | "
                #       f"target_apex_z={target_apex_z:.1f} agent_ceiling={agent_ceiling} | "
                #       f"z0={z0:.1f} z1={z1:.1f} flight_time={flight_time:.1f} v_x={v_x:.1f}")
...
# ensures waypoints are connected with real ballistic arc or straight-line/parabolic-shaped fallback
def get_arc_with_fallback(n0, n1, agent_ceiling, terrain, grid_res, g, agent_id=None, hop_idx=None):
    if n0 == n1:
        r, c = n0
        return ([c * grid_res] * 30, [r * grid_res] * 30, [terrain[n0]] * 30)
    is_clear, arc, _, _ = check_ballistic_hop(n0, n1, terrain, grid_res, g, search_mode=False, agent_ceiling=agent_ceiling)
    if arc is not None and "x" in arc:
        return arc["x"], arc["y"], arc["z"]
    tag = f"Rover {agent_id} hop {hop_idx}" if agent_id is not None else "hop"
    print(f"[Warning] {tag} ({n0}->{n1}) failed fine verification; using fallback arc.")


    x0f, y0f = n0[1] * GRID_RESOLUTION, n0[0] * GRID_RESOLUTION
    x1f, y1f = n1[1] * GRID_RESOLUTION, n1[0] * GRID_RESOLUTION
    z0f, z1f = terrain_map[n0], terrain_map[n1]
    n_steps = 50
    xs, ys, zs = [], [], []
    for k in range(n_steps + 1):
        r = k / n_steps
        xs.append(x0f + r*(x1f-x0f))
        ys.append(y0f + r*(y1f-y0f))
        zs.append(z0f + r*(z1f-z0f) + 400.0*np.sin(np.pi*r))
    return xs, ys, zs
...
def get_neighbors(node, map_size, end_node):
    ...
    # r_curr, c_curr = node
    # goal_vec = np.array([end_node[0] - r_curr, end_node[1] - c_curr])
    # goal_dist = np.linalg.norm(goal_vec)


    # # only applies directional filters if not close to goal
    # if goal_dist > 1e-6:
    #     goal_norm = goal_vec / goal_dist
    #     filtered = []
    #     for (r_n, c_n), dist in neighbors:
    #         step_vec = np.array([r_n - r_curr, c_n - c_curr])
    #         step_norm = step_vec / (np.linalg.norm(step_vec) + 1e-9)
    #         if np.dot(step_norm, goal_norm) > -0.3:
    #             filtered.append(((r_n, c_n), dist))
    #     neighbors = filtered


    dist_to_goal = np.sqrt((end_node[0]-node[0])**2 + (end_node[1]-node[1])**2) * GRID_RESOLUTION
    # dist_to_goal = goal_dist * GRID_RESOLUTION
...
def waypoint_path_planning(start, goal, agent_id, global_reservation_table):
    ...
    """
    fuel_capacity = ROVER_FUEL_CAPACITY[agent_id]
    open_set = []
    ...
    while open_set:
        if time.time() - start_perf_time > 120.0:
            print(f"\nAborting search after 120s timeout; likely not feasible due to constraints.")
            ...
            return None, None, search_history
        ...
        if loop_counter % 1000 == 0:
            ...
            dist_to_goal_now = np.sqrt((current[0]-goal[0])**2 + (current[1]-goal[1])**2) * GRID_RESOLUTION
            print(f"[Best So Far] current node {current}, t={t_step}, dist_to_goal={dist_to_goal_now:.0f}m")
        ...
        for neighbor, distance in get_neighbors(current, MAP_SIZE, goal):
            ...
            # movement_cost = (distance_cost * waypoint_distance_weight) + HOP_OVERHEAD
            ...
            communication_penalty = (1.0 - communication_map[r_new, c_new]) * distance * waypoint_communication_weight
            illumination_penalty = (1.0 - illumination_map[r_new, c_new]) * distance * waypoint_illumination_weight
            ...
            if tentative_g_score < g_score.get((neighbor, next_t), float('inf')):
                ...
                # h_score = (straight_line_remaining * waypoint_distance_weight) + (estimated_hops_remaining * HOP_OVERHEAD)
...
def simulate_tactical_flight_mission(all_agent_strategic_paths):
    ...
    for agent_id, st_path in all_agent_strategic_paths.items():
        ...
        for idx in range(len(st_path) - 1):
            n0, _ = st_path[idx]
            n1, _ = st_path[idx + 1]
            xs, ys, zs = get_arc_with_fallback(n0, n1, agent_ceiling_limit, terrain_map, GRID_RESOLUTION, GRAVITY_MOON, agent_id, idx)


            time_series_x.extend(xs)
            time_series_y.extend(ys)
            time_series_z.extend(zs)
            ...
            #     continue
            # is_clear, arc, _, _ = check_ballistic_hop(n0, n1, terrain_map, GRID_RESOLUTION, GRAVITY_MOON, search_mode=False, agent_ceiling=agent_ceiling_limit)
            # if arc is not None and "x" in arc:
            #     time_series_x.extend(arc["x"])
            #     time_series_y.extend(arc["y"])
            #     time_series_z.extend(arc["z"])
            # else:
            #     print(f"[Warning] Rover {agent_id} hop{idx} ({n0}->{n1}) failed fine verification; use straight-line fallback in animation.")
            #     # print(f"[Debug] hop {idx} {n0}->{n1}: is_clear={is_clear}, arc_is_none={arc is None}")
            #     x0f, y0f = n0[1] * GRID_RESOLUTION, n0[0] * GRID_RESOLUTION
            #     x1f, y1f = n1[1] * GRID_RESOLUTION, n1[0] * GRID_RESOLUTION
            #     z0f, z1f = terrain_map[n0], terrain_map[n1]
            #     fallback_n = 50
            #     for k in range(fallback_n + 1):
            #         r = k / fallback_n
            #         time_series_x.append(x0f + r*(x1f-x0f))
            #         time_series_y.append(y0f + r*(y1f-y0f))
            #         time_series_z.append(z0f + r*(z1f-z0f) + 400.0*np.sin(np.pi*r))


        print(f"[Debug] Rover {agent_id} total trajectory points: {len(time_series_x)}")
...
def predict_future_collisions(agent_continuous_trajectories, t_now, lookahead=LOOKAHEAD_INDICES):
    warnings = []
    agents = list(agent_continuous_trajectories.keys())
    for i in range(len(agents)):
        for j in range(i+1, len(agents)):
            a1, a2 = agents[i], agents[j]
            x1, y1, z1 = agent_continuous_trajectories[a1]
            x2, y2, z2 = agent_continuous_trajectories[a2]
            for dt in range(0, lookahead, 5):
                t = t_now + dt
                if t < len(x1) and t < len(x2):
                    p1 = np.array([x1[t], y1[t], z1[t]])
                    p2 = np.array([x2[t], y2[t], z2[t]])
                    if np.linalg.norm(p1 - p2) < TACTICAL_SAFETY_RADIUS:
                        warnings.append((a1, a2, dt))
                        break
    return warnings
...
mission_profiles = {
    1: {"start": (25, 25), "goal": (125, 125), "color": 'red'}, # southwest to northeast
    2: {"start": (25, 125), "goal": (125, 25), "color": 'blue'}, # northwest to southeast
    3: {"start": (15, 70), "goal": (135, 70), "color": 'purple'}, # south to north
    4: {"start": (90, 15), "goal": (110, 135), "color": 'orange'} # west to east
}
...
check_ballistic_hop_cached.cache_clear()
print(check_ballistic_hop_cached.cache_info())
...
for agent_id, profile in mission_profiles.items():
    ...
    toc = time.time()
    print(f"Rover {agent_id} cache state: {check_ballistic_hop_cached.cache_info()}")
...
ax_anim.plot_surface(X, Y, terrain_map, cmap='terrain', alpha=0.3)


scatters = {}
trail_lines = {}
full_path_lines = {}
for aid in agent_continuous_trajectories:
    color = mission_profiles[aid]["color"]
    xs, ys, zs = agent_continuous_trajectories[aid]


    full_path_lines[aid], = ax_anim.plot(xs, ys, zs, color=color, alpha=0.25, linewidth=1.5)
    trail_lines[aid], = ax_anim.plot([], [], [], color=color, linewidth=2.5, label=f"Rover {aid}")
    scatters[aid], = ax_anim.plot([], [], [], 'o', color=color, markersize=10)


    if len(xs) > 0:
        ax_anim.scatter([xs[0]], [ys[0]], [zs[0]], color='blue', s=90, edgecolors='black', depthshade=False, zorder=15)
        ax_anim.scatter([xs[-1]], [ys[-1]], [zs[-1]], color='magenta', s=90, edgecolors='black', depthshade=False, zorder=15)


warning_text = ax_anim.text2D(0.55, 0.95, "", transform=ax_anim.transAxes, color='red', fontsize=12)
ax_anim.set_xlim(0, MAP_SIZE * GRID_RESOLUTION)
ax_anim.set_ylim(0, MAP_SIZE * GRID_RESOLUTION)
ax_anim.set_zlim(0, MAX_ALLOWABLE_ALTITUDE + 1000.0)
ax_anim.set_title("Tactical Deconfliction Live Flight Paths")
ax_anim.legend(loc="upper left")


max_frames = max(len(v[0]) for v in agent_continuous_trajectories.values())


def update(t):
    active_warnings = predict_future_collisions(agent_continuous_trajectories, t)
    for aid, (xs, ys, zs) in agent_continuous_trajectories.items():
        if t < len(xs):
            scatters[aid].set_data([xs[t]], [ys[t]])
            scatters[aid].set_3d_properties([zs[t]])


            trail_lines[aid].set_data(xs[:t+1], ys[:t+1])
            trail_lines[aid].set_3d_properties(zs[:t+1])
    if active_warnings:
        a1, a2, dt = active_warnings[0]
        warning_text.set_text(f"Rover {a1} and {a2}: collision risk in {dt} sec.")
    else:
...
if max_frames == 0:
    print("No valid continuous trajectory data available. Animation skipped.")
else:
    ani = FuncAnimation(fig_anim, update, frames=range(0, max_frames, 2), interval=50, blit=False)
    plt.show()
...
def visualize_3d_trajectory(waypoints, terrain_map, grid_res=1000.0, g=1.62):
    ...
    for agent_id, path_tuples in all_agent_strategic_paths.items():
        agent_first_arc = True
        agent_ceiling_limit = ROVER_CEILING_LAYERS[agent_id]
        for i in range(len(path_tuples)-1):
            n0, _ = path_tuples[i]
            n1, _ = path_tuples[i+1]
            if n0 == n1:
                continue
            xs, ys, zs = get_arc_with_fallback(n0, n1, agent_ceiling_limit, terrain_map, grid_res, g, agent_id, i)
            ax.plot(xs, ys, zs, color=mission_profiles[agent_id]["color"], linewidth=2, zorder=10, label=f"Rover {agent_id} Arc" if agent_first_arc else "")
            agent_first_arc = False
    ...
    #         if arc_data is None:
    #             print(f"Rover {agent_id} hop {i} ({start}-->{end}) failed fine-grained verification despite passing coarse A* search. Hop may be unsafe.")
    #             continue
    ...
    #             ax.plot(x_traj, y_traj, z_traj, color=mission_profiles[agent_id]["color"], linewidth=2, linestyle='-', zorder=10, label=f"Rover {agent_id} Arc" if arc_labeled else "")
    #             arc_labeled = False


    #             apex = max(arc_data["z"])
    #             print(f"Rover {agent_id} hop {i}: apex = {apex:.1f}m (ceiling={ROVER_CEILING_LAYERS[agent_id]})")
...
axs[0, 0].set_title("Route Planning on Terrain Map")
...
axs[1, 0].set_title(f"Surface Slope Map")
